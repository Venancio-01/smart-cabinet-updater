    ...
    var obj = {
        ...
        val: function (filter, formdate) {
            ...
            if (!!formdate) {
                for (var key in formdate) {
                    ...
                    if ($id.hasClass("rfidFind")) {
                        var rfidelement = $id.parent();
                        if (!!value) {
                            $(element).find('#doc_rfid_1').remove();
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                RFIDHtml = RFIDHtml + '         <input type="text" id="doc_rfid' + divSum + '" name="doc_rfid' + divSum + '"  maxlength="50" lay-verify="required|RfidReplicated" value="' + data[i] + '" class="layui-input"  placeholder="载体标签"> ';
        ...
        authorizeFields: function (filter) {
            ...
            if (!top.clients || !top.clients.moduleFields) {
                element.find('input,select,textarea,ul').each(function (r) {
                    ...
                    if (dataJson != undefined) {
                        for (var i = 0; i < dataJson.length; i++) {
                            if (id == dataJson[i].F_EnCode) {
                                if ($this[0].hasAttribute('lay-affix')) { // 替换'你要检查的属性名'为实际的属性名
                                    $this.parent().parent().parent().removeClass('layui-hide');
                                }
                                else {
